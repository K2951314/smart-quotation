...
from ..license import get_plan_quota, plan_has_feature
...
def _check_brands_quota(rules: list, store, company_id: str) -> None:
    """检查报价规则数量是否超出「数据归属公司订阅档位」上限。-1 表示不限。

    门控点：POST /api/config 保存时检查 rules 数组长度。
    成员公司配置实际写入 parent（store 内部 resolve_data_company_id），
    故配额必须用数据归属公司的 plan，而非操作公司。
    """
    data_company_id = store.resolve_data_company_id(company_id)
    plan = store.resolve_subscription_plan(data_company_id)
    max_brands = get_plan_quota(plan, "max_brands", -1)
...
def _oldest_deletable(configs: list[dict]) -> dict | None:
    """从最旧往新找第一个非 published 的版本（list_configs 按 id 降序，最旧在末尾）。

    已发布（published）版本不可删除，裁剪时跳过，避免客户侧断供。
    """
...
def _enforce_revision_limit(store, company_id: str, *, after_save: bool = False) -> None:
    """版本历史上限检查：超出时自动删除最旧的非 published 版本（-1 表示不限）。

    抽取为公共函数，供 save_config / import_config / rollback_config 共用，
    防止某个端点漏掉检查导致版本无限累积。

    已发布（published）版本不可删除：裁剪时跳过，宁可超限也不删当前发布版。

    after_save=False（默认，pre-save 用）：确保 len < max_revs，为新版本腾位。
    after_save=True（post-save 用，如 rollback）：裁剪到 max_revs。
    """
    # 版本历史配额用数据归属公司的 plan（成员公司配置写入 parent）
    data_company_id = store.resolve_data_company_id(company_id)
    max_revs = get_plan_quota(store.resolve_subscription_plan(data_company_id), "max_config_revisions", -1)
    if max_revs < 0:
        return
    ...
    if after_save:
        ...
        while len(existing) > max_revs:
            oldest = _oldest_deletable(existing)
            if oldest is None:
                break
            try:
...
def register(app) -> None:
    ...
    def list_audit(
        ...
        # 功能门控 + 保留天数：均用「数据归属公司」的 plan。
        # 审计事件按数据归属公司（parent）记录，门控/天数归属须与之一致，
        # 否则成员公司查不到自己的审计记录或天数过滤错乱。
        data_company_id = store.resolve_data_company_id(company_id)
        plan = store.resolve_subscription_plan(data_company_id)
        if not plan_has_feature(plan, "audit_log"):
            raise HTTPException(
                status_code=403,
                detail="审计日志是付费功能，请升级到个人版或专业版订阅。",
            )
        # 按订阅档位过滤审计日志保留天数（-1 或 0 表示不过滤）
        days = get_plan_quota(plan, "audit_log_days", 7)
